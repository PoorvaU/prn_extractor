...
def fetch_tables(conn, Dept_no, Dept_Code):
    ...
    sorted_tables = sorted(tables_list, key=lambda x: int(x.split('_')[-1].replace('fe', '1').replace('se', '2').replace('te', '3').replace('be', '4')))

    return sorted_tables


def create_and_download_excel(df, file_name):
    ...
    if conn:
        ...
        if export_type == 'Institute wise' or export_type == 'Department wise':
                ...
                if export_type == 'Institute wise':
                    if st.button("Export"):
                        ...
                        if combined_data:
                            ...
                            create_and_download_excel(combined_df, f"{Dept_no}_{Dept_Code}_Institute_Wise")
            ...
            if class_name:
                ...
                if st.button("Export"):
                    if tables:
                        ...
                        if combined_data:
                            combined_df = pd.concat(combined_data, ignore_index=True)
                            create_and_download_excel(combined_df, f"{class_name}_Year_Institute_Wise")
                    else:
            ...
            if selected_table and st.button("Export"):
                ...
                if not df.empty:
                    create_and_download_excel(df, selected_table)
                else:
                    st.warning("Selected table is empty or does not exist.")
