...
def main():
    ...
    if selected_department == "FE Department":
        ...
        if uploaded_file:
            ...
            excel_columns = list(df_excel.columns)
            selected_excel_column = st.selectbox("Select a column for comparison from Excel file", excel_columns, key="excel_column_selectbox")
                ...
                if selected_table:
                    ...
                    if selected_db_column:
                        ...
                        if st.button("Run Comparison"):
                            ...
                            # Step 7: Save matched records into a new table
                            new_table_name = f"{selected_table}_branchwise_comparison"
                            cursor = conn.cursor()
                            cursor.execute(f"SHOW TABLES LIKE '{new_table_name}'")
                            if cursor.fetchone():
                                st.warning(f"Table '{new_table_name}' already exists. Skipping table creation.")
                    ...
                    st.error("Failed to connect to the database.")
    elif selected_department == "Other Departments":
        st.write("Comparison logic for other departments goes here.")
