...
def get_db_connection():
    try:
        conn = mysql.connector.connect(
            host=st.secrets["database"]["DATABASE_HOST"],
            user=st.secrets["database"]["DATABASE_USER"],
            password=st.secrets["database"]["DATABASE_PASSWORD"],
            database=st.secrets["database"]["DATABASE_NAME"],
            port=st.secrets["database"]["DATABASE_PORT"]
        )
        return conn
...
def check_existing_records(conn, table_name, column_name, values):
    cursor = conn.cursor()
...
def append_data_to_table(conn, table_name, data):
    cursor = conn.cursor()
    try:
        ...
        existing_records = check_existing_records(conn, table_name, "Student's Enrollment Number", data["Student's Enrollment Number"].tolist())
...
def main():
    ...
    if selected_department == "FE Department":
                    ...
                    if selected_db_column:
                        ...
                        if "Dept_name" in department_table_data.columns:
                            ...
                            if st.button("Run Comparison"):
                                ...
                                existing_records = check_existing_records(conn, new_table_name, selected_db_column, matched_df[selected_db_column].tolist())
        ...
        if uploaded_file:
            ...
            excel_columns = list(df_excel.columns)
            selected_excel_column = st.selectbox("Select a column from Excel file", excel_columns, key="excel_column_selectbox_fe")

            # Step 3: Connect to MySQL and list tables
            conn = get_db_connection()
            if conn and conn.is_connected():
                tables = get_table_names(conn)
                selected_table = st.selectbox("Select a table from the database", tables, key="db_table_selectbox_fe")

                if selected_table:
                    # Step 4: List columns from selected table
                    db_columns = get_column_names(conn, selected_table)
                    selected_db_column = st.selectbox("Select a column from the database table", db_columns, key="db_column_selectbox_fe")

                    if selected_db_column:
                        # Step 5: Fetch data from the selected table
                        db_data = get_table_data(conn, selected_table)

                        # Step 6: Select branch and create new table name
                        branch_table_data = get_table_data(conn, "Branch")
                        st.write("Branch table columns:", branch_table_data.columns)

                        if "Branch_name" in branch_table_data.columns:
                            branches = branch_table_data["Branch_name"].tolist()
                            selected_branch = st.selectbox("Select a branch", branches, key="branch_selectbox")

                            # Find the corresponding Branch_no and Branch_Code
                            branch_info = branch_table_data[branch_table_data["Branch_name"] == selected_branch].iloc[0]
                            branch_no = branch_info["Branch_no"]
                            Branch_Code = branch_info["Branch_Code"]
                            new_table_name = f"{branch_no}_{Branch_Code}_FE"

                            if st.button("Run Comparison - Branchwise"):
                                # Step 7: Perform comparison and create a new table with matched records
                                matches = []
                                unmatched = []
                                for excel_value in df_excel[selected_excel_column].dropna():
                            ...
                            st.error("The 'Branch_name' column does not exist in the 'Branch' table.")
        ...
        st.write("Other Departments functionality is yet to be implemented.")
